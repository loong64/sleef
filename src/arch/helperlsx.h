/* LoongArch LSX ISA backend. */
#if !defined(__loongarch_sx) && !defined(SLEEF_GENHEADER)
#error Please specify -mlsx.
#endif
...
typedef v4i32 vmask;
typedef v4i32 vopmask;
typedef __m128 vfloat;
typedef v4i32 vint2;
typedef __m128d vdouble;
typedef v4i32 vint;
typedef v2i64 vint64;
typedef v2u64 vuint64;
...
static INLINE vopmask vcast_vo32_vo64(vopmask m) {
  return (vopmask)__lsx_vpickod_w((__m128i)m, (__m128i)m);
}
static INLINE vopmask vcast_vo64_vo32(vopmask m) {
  return (vopmask)__lsx_vilvl_w((__m128i)m, (__m128i)m);
}

static INLINE vint vcast_vi_i(int i) { return (vint)__lsx_vreplgr2vr_w(i); }
static INLINE vint2 vcast_vi2_i(int i) { return (vint2)__lsx_vreplgr2vr_w(i); }
static INLINE vfloat vcast_vf_f(float f) { union { float f; int32_t i; } u = { f }; return (vfloat)__lsx_vreplgr2vr_w(u.i); }
static INLINE vdouble vcast_vd_d(double d) { union { double d; int64_t i; } u = { d }; return (vdouble)__lsx_vreplgr2vr_d(u.i); }
...
static INLINE vint2 vcastu_vi2_vi(vint vi) {
  return (vint2)__lsx_vilvl_w((__m128i)vi, __lsx_vreplgr2vr_w(0));
}
static INLINE vint vcastu_vi_vi2(vint2 vi2) {
  return (vint)__lsx_vpickod_w((__m128i)vi2, (__m128i)vi2);
}

static INLINE vint vreinterpretFirstHalf_vi_vi2(vint2 vi2) { return (vint)vi2; }
static INLINE vint2 vreinterpretFirstHalf_vi2_vi(vint vi) {
  return (vint2)__lsx_vpickev_d(__lsx_vreplgr2vr_d(0), (__m128i)vi);
}

static INLINE vdouble vrev21_vd_vd(vdouble vd) { return (vdouble)__lsx_vshuf4i_w((__m128i)vd, 0x4e); }
static INLINE vdouble vreva2_vd_vd(vdouble vd) { return vd; }
static INLINE vfloat vrev21_vf_vf(vfloat vd) { return (vfloat)__lsx_vshuf4i_w((__m128i)vd, 0xb1); }
static INLINE vfloat vreva2_vf_vf(vfloat vd) { return (vfloat)__lsx_vshuf4i_w((__m128i)vd, 0x4e); }
#ifdef ENABLE_LONGDOUBLE
...
static INLINE int vtestallones_i_vo64(vopmask g) {
  return __lsx_bnz_d((__m128i)g);
}

static INLINE int vtestallones_i_vo32(vopmask g) {
  return __lsx_bnz_w((__m128i)g);
}

//

static vint2 vloadu_vi2_p(int32_t *p) {
  return (vint2)__lsx_vld(p, 0);
}
...
static vint vloadu_vi_p(int32_t *p) {
  return (vint)__lsx_vldrepl_d(p, 0);
}

static void vstoreu_v_p_vi(int32_t *p, vint v) {
  __lsx_vstelm_d((__m128i)v, p, 0, 0);
}
...
static INLINE vmask vandnot_vm_vm_vm(vmask x, vmask y) {
  return (vmask)__lsx_vandn_v((__m128i)x, (__m128i)y);
}
...
static INLINE vopmask vandnot_vo_vo_vo(vopmask x, vopmask y) {
  return (vopmask)__lsx_vandn_v((__m128i)x, (__m128i)y);
}
...
static INLINE vdouble vsel_vd_vo_vd_vd(vopmask o, vdouble x, vdouble y) {
  return (vdouble)__lsx_vbitsel_v((__m128i)y, (__m128i)x, (__m128i)o);
}
static INLINE vint2 vsel_vi2_vo_vi2_vi2(vopmask o, vint2 x, vint2 y) {
  return (vint2)__lsx_vbitsel_v((__m128i)y, (__m128i)x, (__m128i)o);
}
...
static INLINE vdouble vcast_vd_vi(vint vi) { return __lsx_vffintl_d_w((__m128i)vi); }
static INLINE vint vtruncate_vi_vd(vdouble vd) { return (vint)__lsx_vftintrz_w_d(vd, vd); }
static INLINE vint vrint_vi_vd(vdouble vd) { return (vint)__lsx_vftintrne_w_d(vd, vd); }
static INLINE vdouble vtruncate_vd_vd(vdouble vd) { return __lsx_vfrintrz_d(vd); }
static INLINE vdouble vrint_vd_vd(vdouble vd) { return __lsx_vfrintrne_d(vd); }

static INLINE vopmask veq64_vo_vm_vm(vmask x, vmask y) {
  return (vopmask)__lsx_vseq_d((__m128i)x, (__m128i)y);
}

static INLINE vmask vadd64_vm_vm_vm(vmask x, vmask y) { return (vmask)__lsx_vadd_d((__m128i)x, (__m128i)y); }

static INLINE vmask vcast_vm_i64(int64_t i) { return (vmask)__lsx_vreplgr2vr_d(i); }
static INLINE vmask vcast_vm_u64(uint64_t i) { return (vmask)__lsx_vreplgr2vr_d((long long)i); }
static INLINE vmask vcastu_vm_vi(vint vi) {
  return (vmask)__lsx_vilvl_w((__m128i)vi, __lsx_vreplgr2vr_w(0));
}
static INLINE vint vcastu_vi_vm(vmask vm) {
  return (vint)__lsx_vpickod_w((__m128i)vm, (__m128i)vm);
}
static INLINE vmask vcast_vm_vi(vint vi) {
  return (vmask)__lsx_vsllwil_d_w((__m128i)vi, 0);
}
static INLINE vint vcast_vi_vm(vmask vm) {
  return (vint)__lsx_vpickev_w((__m128i)vm, (__m128i)vm);
}
static INLINE vmask vsel_vm_vo64_vm_vm(vopmask m, vmask x, vmask y) {
  return (vmask)__lsx_vbitsel_v((__m128i)y, (__m128i)x, (__m128i)m);
}
static INLINE vmask vsub64_vm_vm_vm(vmask x, vmask y) { return (vmask)__lsx_vsub_d((__m128i)x, (__m128i)y); }
static INLINE vmask vneg64_vm_vm(vmask x) { return (vmask)__lsx_vneg_d((__m128i)x); }
static INLINE vopmask vgt64_vo_vm_vm(vmask x, vmask y) { return (vopmask)__lsx_vslt_d((__m128i)y, (__m128i)x); }
#define vsll64_vm_vm_i(x, c) ((vmask)__lsx_vslli_d((__m128i)(x), c))
//@#define vsll64_vm_vm_i(x, c) ((vmask)__lsx_vslli_d((__m128i)(x), c))
#define vsrl64_vm_vm_i(x, c) ((vmask)__lsx_vsrli_d((__m128i)(x), c))
//@#define vsrl64_vm_vm_i(x, c) ((vmask)__lsx_vsrli_d((__m128i)(x), c))
...
static INLINE vdouble vabs_vd_vd(vdouble d) { return (vdouble)__lsx_vbitclri_d((__m128i)d, 63); }
static INLINE vdouble vneg_vd_vd(vdouble d) { return (vdouble)__lsx_vbitrevi_d((__m128i)d, 63); }
#if CONFIG == 1
...
static INLINE vdouble vmlanp_vd_vd_vd_vd(vdouble x, vdouble y, vdouble z) { return __lsx_vfmadd_d(vneg_vd_vd(x), y, z); }
...
static INLINE vdouble vfmanp_vd_vd_vd_vd(vdouble x, vdouble y, vdouble z) { return __lsx_vfmadd_d(vneg_vd_vd(x), y, z); }
...
static INLINE vopmask veq_vo_vd_vd(vdouble x, vdouble y) { return (vopmask)__lsx_vfcmp_ceq_d(x, y); }
static INLINE vopmask vneq_vo_vd_vd(vdouble x, vdouble y) { return (vopmask)__lsx_vfcmp_cune_d(x, y); }
static INLINE vopmask vlt_vo_vd_vd(vdouble x, vdouble y) { return (vopmask)__lsx_vfcmp_clt_d(x, y); }
static INLINE vopmask vle_vo_vd_vd(vdouble x, vdouble y) { return (vopmask)__lsx_vfcmp_cle_d(x, y); }
static INLINE vopmask vgt_vo_vd_vd(vdouble x, vdouble y) { return (vopmask)__lsx_vfcmp_clt_d(y, x); }
static INLINE vopmask vge_vo_vd_vd(vdouble x, vdouble y) { return (vopmask)__lsx_vfcmp_cle_d(y, x); }
...
static INLINE vopmask visnan_vo_vd(vdouble d) { return (vopmask)__lsx_vfcmp_cune_d(d, d); }

static INLINE vdouble vsqrt_vd_vd(vdouble d) {
#if !defined(SLEEF_ENABLE_ALTSQRT)
  return __lsx_vfsqrt_d(d);
#else
...
static INLINE vdouble vload_vd_p(const double *ptr) { return (vdouble)__lsx_vld(ptr, 0); }
static INLINE vdouble vloadu_vd_p(const double *ptr) {
  return (vdouble)__lsx_vld(ptr, 0);
}
...
static INLINE void vstore_v_p_vd(double *ptr, vdouble v) { __lsx_vst((__m128i)v, ptr, 0); }
static INLINE void vstoreu_v_p_vd(double *ptr, vdouble v) {
  __lsx_vst((__m128i)v, ptr, 0);
}
...
static INLINE vfloat vsel_vf_vo_vf_vf(vopmask o, vfloat x, vfloat y) {
  return (vfloat)__lsx_vbitsel_v((__m128i)y, (__m128i)x, (__m128i)o);
}
...
static INLINE vfloat vcast_vf_vi2(vint2 vi) { return __lsx_vffint_s_w((__m128i)vi); }

static INLINE vint2 vtruncate_vi2_vf(vfloat vf) { return (vint2)__lsx_vftintrz_w_s(vf); }

static INLINE vint2 vrint_vi2_vf(vfloat vf) { return (vint2)__lsx_vftintrne_w_s(vf); }
static INLINE vfloat vtruncate_vf_vf(vfloat vd) { return __lsx_vfrintrz_s(vd); }
static INLINE vfloat vrint_vf_vf(vfloat vd) { return __lsx_vfrintrne_s(vd); }
...
static INLINE vfloat vabs_vf_vf(vfloat f) { return (vfloat)__lsx_vbitclri_w((__m128i)f, 31); }
static INLINE vfloat vneg_vf_vf(vfloat d) { return (vfloat)__lsx_vbitrevi_w((__m128i)d, 31); }
#if CONFIG == 1
static INLINE vfloat vmla_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return __lsx_vfmadd_s(x, y, z); }
// FNMSUB computes -(x*y-z), which has different signed-zero semantics
// from the required fma(-x, y, z).
static INLINE vfloat vmlanp_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return __lsx_vfmadd_s(vneg_vf_vf(x), y, z); }
...
static INLINE vfloat vfmanp_vf_vf_vf_vf(vfloat x, vfloat y, vfloat z) { return __lsx_vfmadd_s(vneg_vf_vf(x), y, z); }
...
static INLINE vopmask veq_vo_vf_vf(vfloat x, vfloat y) { return (vopmask)__lsx_vfcmp_ceq_s(x, y); }
static INLINE vopmask vneq_vo_vf_vf(vfloat x, vfloat y) { return (vopmask)__lsx_vfcmp_cune_s(x, y); }
static INLINE vopmask vlt_vo_vf_vf(vfloat x, vfloat y) { return (vopmask)__lsx_vfcmp_clt_s(x, y); }
static INLINE vopmask vle_vo_vf_vf(vfloat x, vfloat y) { return (vopmask)__lsx_vfcmp_cle_s(x, y); }
static INLINE vopmask vgt_vo_vf_vf(vfloat x, vfloat y) { return (vopmask)__lsx_vfcmp_clt_s(y, x); }
static INLINE vopmask vge_vo_vf_vf(vfloat x, vfloat y) { return (vopmask)__lsx_vfcmp_cle_s(y, x); }
...
static INLINE vint2 vandnot_vi2_vi2_vi2(vint2 x, vint2 y) {
  return (vint2)__lsx_vandn_v((__m128i)x, (__m128i)y);
}
...
#define vsll_vi2_vi2_i(x, c) ((vint2)__lsx_vslli_w((__m128i)(x), (c)))
//@#define vsll_vi2_vi2_i(x, c) ((vint2)__lsx_vslli_w((__m128i)(x), (c)))
#define vsrl_vi2_vi2_i(x, c) ((vint2)__lsx_vsrli_w((__m128i)(x), (c)))
//@#define vsrl_vi2_vi2_i(x, c) ((vint2)__lsx_vsrli_w((__m128i)(x), (c)))
#define vsra_vi2_vi2_i(x, c) ((vint2)__lsx_vsrai_w((__m128i)(x), (c)))
//@#define vsra_vi2_vi2_i(x, c) ((vint2)__lsx_vsrai_w((__m128i)(x), (c)))

static INLINE vopmask veq_vo_vi2_vi2(vint2 x, vint2 y) { return (vopmask)(x == y); }
static INLINE vopmask vgt_vo_vi2_vi2(vint2 x, vint2 y) { return (vopmask)__lsx_vslt_w((__m128i)y, (__m128i)x); }
static INLINE vint2 veq_vi2_vi2_vi2(vint2 x, vint2 y) { return x == y; }
static INLINE vint2 vgt_vi2_vi2_vi2(vint2 x, vint2 y) { return (vint2)__lsx_vslt_w((__m128i)y, (__m128i)x); }
...
static INLINE vopmask visnan_vo_vf(vfloat d) { return (vopmask)__lsx_vfcmp_cune_s(d, d); }

static INLINE vfloat vsqrt_vf_vf(vfloat d) {
#if !defined(SLEEF_ENABLE_ALTSQRT)
  return __lsx_vfsqrt_s(d);
#else
...
static INLINE vfloat vload_vf_p(const float *ptr) { return (vfloat)__lsx_vld(ptr, 0); }
static INLINE vfloat vloadu_vf_p(const float *ptr) {
  return (vfloat)__lsx_vld(ptr, 0);
}
...
static INLINE void vstore_v_p_vf(float *ptr, vfloat v) { __lsx_vst((__m128i)v, ptr, 0); }
